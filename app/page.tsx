...
export default function PortalRH() {
  ...
  const [lembrarLogin, setLembrarLogin] = useState(false); // NOVO: Estado para Lembrar Login
  ...
  // NOVO: Recupera o login salvo ao abrir a página
  useEffect(() => {
    const loginSalvo = localStorage.getItem('dinamo_lembrar_login');
    if (loginSalvo) {
      setLoginInput(loginSalvo);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // NOVO: Lógica de Lembrar Login
        if (lembrarLogin) {
          localStorage.setItem('dinamo_lembrar_login', loginInput);
        } else {
          localStorage.removeItem('dinamo_lembrar_login');
          setLoginInput('');
        }
      } else {
  ...
  // NOVO: Ação ao clicar em Esqueceu a Senha
  const handleEsqueceuSenha = () => {
    alert("Como o acesso é restrito, por favor, entre em contato com o Administrador do Sistema (RH/SESMT) para solicitar a redefinição da sua senha.");
  };
  ...
  const buscarColaboradorParaEmissao = async (matriculaAlvo: string) => {
    ...
    try {
      ...
      } else { setColaborador(null); setErroEmissao('Matrícula não encontrada.'); }
  ...
  const abrirModalSenha = () => { setMostrarModalSenha(true); setNovaSenha(''); setConfirmarSenha(''); setErroSenha(''); setSucessoSenha(''); };
        ...
        <div className="z-10 w-full max-w-md p-8">
          <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl shadow-slate-200/50 overflow-hidden border border-white/50">
            ...
            <form onSubmit={handleLogin} className="p-8 flex flex-col gap-6 bg-white">
              ...
              {/* NOVO: LEMBRAR LOGIN E ESQUECEU A SENHA */}
              <div className="flex items-center justify-between mt-[-10px] mb-2 px-1">
                <label className="flex items-center gap-2 text-xs font-semibold text-slate-500 cursor-pointer hover:text-slate-700 transition-colors">
                  <input type="checkbox" checked={lembrarLogin} onChange={(e) => setLembrarLogin(e.target.checked)} className="w-3.5 h-3.5 accent-[#0a84ff] rounded border-slate-300" />
                  Lembrar login
                </label>
                <button type="button" onClick={handleEsqueceuSenha} className="text-xs font-bold text-[#0a84ff] hover:text-blue-700 transition-colors">
                  Esqueceu a senha?
                </button>
              </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden print-main-adjust bg-[#f8fafc]">
        ...
        <header className="h-24 bg-white shadow-sm border-b border-slate-200 flex items-center justify-between px-6 md:px-10 z-10 hide-on-print relative">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={abrirModalSenha} className="w-10 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-800 transition-colors flex items-center justify-center" title="Alterar minha senha">
                ...
                <div className="flex flex-col xl:flex-row gap-8">
                  ...
                  <div className="xl:col-span-2 bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-full">
                    <div className="flex justify-between items-center mb-6">
                      <h3 className="text-lg font-bold text-slate-800">Visualização do Crachá</h3>
                      ...
                      <div className="cracha-card w-[54mm] h-[86mm] bg-white relative p-[2mm] flex flex-col box-border" style={{ border: '1px solid #ccc' }}>
                        <div className="mt-[2mm] w-full flex flex-col gap-[3mm]">
                          ...
                          <div className="flex w-full gap-[2mm]">
                            ...
                            {/* AJUSTE VISUAL: TIPO SANGUÍNEO COM FUNDO BRANCO E BORDA PRETA, TEXTO PRETO */}
                            <div className="relative border-[1.5px] border-black rounded-[4px] h-[7mm] w-[14mm] flex items-center justify-center bg-white"><span className="absolute -top-[2.5mm] left-[1mm] bg-white px-[0.5mm] text-[5px] font-bold text-black leading-none">Tp. Sangue</span><div className="text-[8px] text-black font-semibold uppercase">{colaborador.tipo_sanguineo || ''}</div></div>
                          </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col border-t-4 border-[#f39c12]">
              ...
              <form onSubmit={salvarEdicao} className="p-6 flex flex-col gap-6 overflow-y-auto max-h-[75vh] bg-slate-50/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
                  <div>
                    <label className="block text-sm font-semibold text-slate-800 mb-2"><i className="fas fa-tint mr-1"></i> Tipo Sanguíneo</label>
                    <input type="text" value={colaboradorEditando.tipo_sanguineo || ''} onChange={e => setColaboradorEditando({...colaboradorEditando, tipo_sanguineo: e.target.value})} placeholder="Ex: O+" className="w-full bg-white border border-slate-300 p-3.5 rounded-2xl focus:outline-none focus:border-[#0a84ff] focus:ring-2 focus:ring-blue-100 text-slate-800 font-medium uppercase transition-all shadow-sm" />
                  </div>
                  <div>
        ...
        {mostrarModalSenha && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 hide-on-print">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden border-t-4 border-[#0a84ff]">
              <div className="p-5 border-b border-slate-100 flex justify-between"><h3 className="font-bold text-slate-800 flex items-center gap-2"><i className="fas fa-key text-[#0a84ff]"></i>Alterar Senha</h3><button onClick={() => setMostrarModalSenha(false)}><i className="fas fa-times text-slate-400"></i></button></div>
              <form onSubmit={handleAlterarSenha} className="p-6 flex flex-col gap-4">
                ...
                <div><label className="text-xs font-semibold text-slate-500 block mb-1">Nova Senha</label><input type="password" required value={novaSenha} onChange={e => setNovaSenha(e.target.value)} className="w-full border border-slate-300 rounded-xl p-3 text-sm focus:outline-none focus:border-[#0a84ff]" placeholder="••••••••" /></div>
                <div><label className="text-xs font-semibold text-slate-500 block mb-1">Confirmar Senha</label><input type="password" required value={confirmarSenha} onChange={e => setConfirmarSenha(e.target.value)} className="w-full border border-slate-300 rounded-xl p-3 text-sm focus:outline-none focus:border-[#0a84ff]" placeholder="••••••••" /></div>
                <button type="submit" disabled={salvandoSenha} className="w-full bg-[#0a84ff] text-white py-3 rounded-xl font-semibold mt-2 shadow-md">Alterar Senha</button>
              </form>
            </div>
          </div>
        )}
