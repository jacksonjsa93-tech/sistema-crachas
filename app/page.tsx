...
export default function PortalRH() {
  ...
  const processarFicheiro = async () => {
    if (!ficheiroExcel) {
      setResultadoMsg({ tipo: 'erro', texto: 'Nenhum ficheiro selecionado.' });
      return;
    }
    ...
    reader.onload = async (e) => {
      try {
        ...
        // 🔍 LOG no console do navegador (F12 para ver)
        ...
        // Função para encontrar coluna mesmo com acentos e maiúsculas/minúsculas
        const getValor = (row: any, chave: string) => {
          const chaveNormalizada = chave
            .normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '')
            .toLowerCase();
          for (const col of Object.keys(row)) {
            const colNormalizada = col
              .normalize('NFD')
              .replace(/[\u0300-\u036f]/g, '')
              .toLowerCase();
            if (colNormalizada === chaveNormalizada) {
              return row[col];
            }
          }
          return undefined;
        ...
        const payload = json
          .map((row) => {
            const matricula = String(getValor(row, 'matricula') || row['Matricula'] || row['MATRICULA'] || '').trim();
            const nome_completo = String(
              getValor(row, 'nome completo') ||
              row['Nome completo'] ||
              row['NOME COMPLETO'] ||
              row['Nome Completo'] ||
              ''
              ...
              .trim()
              .toUpperCase();
            const cpf = String(getValor(row, 'cpf') || row['CPF'] || '').trim();
            const rg = String(getValor(row, 'rg') || row['RG'] || '').trim();
            const desc_funcao = String(
              getValor(row, 'desc.funcao') ||
              row['Desc.Funcao'] ||
              row['DESC.FUNCAO'] ||
              row['Desc. Funcao'] ||
              ''
            ...
            return { matricula, nome_completo, cpf, rg, desc_funcao };
          })
        ...
        if (payload.length === 0) {
          setResultadoMsg({
            tipo: 'erro',
            texto:
              'Nenhum dado válido encontrado. Verifique se as colunas têm os nomes: Matricula, Nome completo, CPF, RG, Desc.Funcao',
          });
          setImportando(false);
          return;
        }

        setResultadoMsg({ tipo: 'loading', texto: `A enviar ${payload.length} registos...` });
        ...
        if (!response.ok) {
          console.error('❌ Erro do Supabase:', response.status, responseBody);
          throw new Error(`Erro ${response.status}: ${responseBody}`);
        }

        setResultadoMsg({
          tipo: 'sucesso',
          texto: `${payload.length} colaboradores importados/atualizados com sucesso!`,
        });
        ...
        setResultadoMsg({
          tipo: 'erro',
          texto: `Erro: ${error.message || 'Verifique o console (F12) para mais detalhes.'}`,
        });
      } finally {
        setImportando(false);
      }
            ...
            <div className="animation-fade-in max-w-4xl mx-auto hide-on-print">
              <div className="bg-white rounded-xl border border-[#cfd8dc] shadow-sm overflow-hidden">
                <div className="p-6 border-b border-[#eceff1] bg-[#fafafa]">
                  <h3 className="font-bold text-[#023A58] text-lg">
                    <i className="fas fa-upload mr-2"></i> Importação de Colaboradores em Lote
                  </h3>
                  <p className="text-sm text-slate-500 mt-1">
                    Carregue a base de dados do sistema principal (XLSX, CSV) para atualizar o SGSO Premium
                    automaticamente.
                  </p>
                </div>
                ...
                <div className="p-8">
                    ...
                    className={`border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center transition-all cursor-pointer
                      ${
                        arrastando
                          ...
                          : 'border-[#b0bec5] bg-[#f8f9fa] hover:bg-[#eceff1]'
                      }`}
                  >
                  ...
                  {ficheiroExcel && (
                    <div className="mt-8 p-4 bg-[#e8f5e9] border border-[#2ecc71] rounded-lg flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            {(ficheiroExcel.size / 1024).toFixed(2)} KB pronto para processamento.
                          </p>
                        </div>
                      </div>
                      <div className="flex gap-3">
                        <button
                          onClick={() => {
                            setFicheiroExcel(null);
                            setResultadoMsg({ tipo: '', texto: '' });
                          }}
                          disabled={importando}
                          className="px-4 py-2 text-sm font-bold text-[#e74c3c] hover:bg-[#ffebee] rounded-lg transition-colors disabled:opacity-50"
                        >
                          Remover
                        </button>
                        <button
                          onClick={processarFicheiro}
                          disabled={importando}
                          className="px-6 py-2 text-sm font-bold bg-[#2ecc71] text-white rounded-lg hover:bg-[#27ae60] transition-colors shadow-sm disabled:opacity-50 flex items-center gap-2"
                        >
                          {importando ? (
                            <>
                            ...
                            <>
                              <i className="fas fa-database"></i> Enviar para Base
                            </>
                          )}
                        </button>
                      </div>
                  ...
                  {resultadoMsg.texto && (
                    <div
                      className={`mt-6 p-4 rounded-lg flex items-center gap-3 font-medium border
                  ...
                  <div className="mt-8 bg-[#fff8e1] border border-[#f39c12] rounded-lg p-5">
                    <h4 className="font-bold text-[#d35400] mb-2 flex items-center gap-2">
                      <i className="fas fa-info-circle"></i> Padrão Automático Identificado
                    </h4>
                    <p className="text-sm text-[#7f8c8d] mb-3">
                      O sistema já está configurado para ler automaticamente o modelo de extração oficial da Dínamo.
                      Colunas detectadas:
                    </p>
                    <div className="flex flex-wrap gap-2">
                      <span className="bg-white px-3 py-1 rounded text-xs font-bold border border-[#f39c12] text-[#d35400]">
                        Matricula
                      </span>
                      <span className="bg-white px-3 py-1 rounded text-xs font-bold border border-[#f39c12] text-[#d35400]">
                        Nome completo
          ...
          {abaAtiva === 'emissao' && (
            <div className="animation-fade-in max-w-6xl mx-auto">
              <form
                ...
                className="bg-white p-6 rounded-xl border border-[#cfd8dc] shadow-sm mb-8 flex gap-4 items-end hide-on-print"
              >
                <div className="flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-[#f8f9fa] border border-[#b0bec5] rounded-lg px-4 py-3 text-[#263238] focus:outline-none focus:border-[#035B8B]"
                  />
                </div>
                <button
                  type="submit"
                  disabled={carregando}
                  className="bg-[#023A58] text-white font-bold px-8 py-3 rounded-lg hover:bg-[#035B8B] transition-all shadow-sm"
                >
              ...
              {erro && (
                <p className="text-[#e74c3c] mb-6 bg-[#fdeced] p-4 rounded-lg border border-[#f5b7b1] font-medium hide-on-print">
                  {erro}
                </p>
              )}
              ...
              {colaborador && (
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                  <div className="bg-white p-6 rounded-xl border border-[#cfd8dc] shadow-sm flex flex-col items-center hide-on-print">
                    <h3 className="text-lg font-bold text-[#023A58] mb-4 w-full border-b border-[#eceff1] pb-2">
                    ...
                    <div className="w-48 h-64 bg-[#f8f9fa] border-2 border-dashed border-[#b0bec5] rounded-lg overflow-hidden relative flex items-center justify-center mb-4">
                      {cameraAtiva ? (
                        ...
                        <span className="text-slate-400 text-xs text-center px-4 font-medium">
                          Nenhuma foto
                          <br />
                          capturada.
                        </span>
                    ...
                    <div className="w-full space-y-2">
                      {cameraAtiva ? (
                        <button
                          onClick={tirarFoto}
                          className="w-full bg-[#2ecc71] hover:bg-[#27ae60] text-white font-bold py-2 rounded-lg shadow-sm"
                        >
                          Capturar Foto
                        </button>
                      ) : (
                        <button
                          onClick={ligarCamera}
                          className="w-full bg-[#035B8B] hover:bg-[#023A58] text-white font-bold py-2 rounded-lg shadow-sm"
                        >
                          <i className="fas fa-camera mr-2"></i> Ligar Câmara
                        </button>
                      )}
                  ...
                  <div className="xl:col-span-2 bg-white p-6 rounded-xl border border-[#cfd8dc] shadow-sm">
                    <div className="flex justify-between items-center mb-6 border-b border-[#eceff1] pb-4 hide-on-print">
                      ...
                      <button
                        ...
                        className={`font-bold px-6 py-2 rounded-lg transition-all shadow-sm ${
                          fotoCapturada
                            ? 'bg-[#023A58] hover:bg-[#035B8B] text-white'
                    ...
                    <div className="flex flex-col md:flex-row gap-8 justify-center items-center bg-[#f8f9fa] p-8 rounded-lg overflow-x-auto print-container border border-[#eceff1]">
                      <div
                        className="cracha-card w-[54mm] h-[86mm] bg-white relative flex flex-col items-center overflow-hidden box-border"
                      ...
                      >
                        ...
                        <div className="mt-[2mm] text-center z-10 w-full px-2">
                          <div className="text-[#051e42] font-black text-[18px] leading-[1.0]" style={{ fontFamily: 'Arial, sans-serif' }}>
                            {formatarNomeCurto(colaborador.nome_completo).split(' ')[0]}
                            <br />
                      ...
                      <div
                        className="cracha-card w-[54mm] h-[86mm] bg-white relative p-[2mm] flex flex-col box-border"
                        style={{ border: '1px solid #ccc' }}
                      >
                        <div className="mt-[2mm] w-full flex flex-col gap-[3mm]">
                          <div className="relative border-[1.5px] border-black rounded-[4px] h-[7mm] flex items-center justify-center w-full">
                            <span className="absolute -top-[2.5mm] left-[2mm] bg-white px-[1mm] text-[6px] font-bold text-black leading-none">
                              Nome
                            </span>
                            <div className="text-[7.5px] text-black font-semibold uppercase">
                          ...
                          <div className="relative border-[1.5px] border-black rounded-[4px] h-[7mm] flex items-center justify-center w-full">
                            ...
                            <div className="text-[8px] text-black font-semibold uppercase">
                              {colaborador.cpf || '000.000.000-00'}
                            </div>
                          </div>
                          <div className="flex w-full gap-[2mm] items-stretch h-[24mm]">
                            <div className="flex flex-col flex-1 justify-between">
                              <div className="relative border-[1.5px] border-black rounded-[4px] h-[7mm] flex items-center justify-center w-full">
                                ...
                                <div className="text-[7px] text-black font-semibold uppercase truncate px-1">
                                  {colaborador.desc_funcao}
                                </div>
                              </div>
                              <div className="relative border-[1.5px] border-black rounded-[4px] h-[7mm] flex items-center justify-center w-full">
                                <span className="absolute -top-[2.5mm] left-[2mm] bg-white px-[1mm] text-[6px] font-bold text-black leading-none">
                              ...
                              <div className="relative border-[1.5px] border-black rounded-[4px] h-[7mm] flex items-center justify-center w-full">
                                ...
                                <div className="text-[8px] text-black font-semibold uppercase">
                                  {String(colaborador.matricula).padStart(8, '0')}
                                </div>
                              </div>
                            </div>
                        ...
                        <div className="absolute bottom-[2mm] left-[2mm] right-[2mm] z-0 flex flex-col items-center p-b[2mm]">
                          <div className="text-[7px] text-black leading-[1.3] mb-[3mm] text-center font-medium w-[47mm]">
                            Em caso de extravio/perda, favor comunicar ao
                            <br />
                            Departamento Pessoal.
                          </div>
                          <div className="text-center w-full mb-[1mm]">
                            <div className="text-[7.5px] font-bold text-black mb-[0.5mm]">www.dinamo.srv.br</div>
                            <div className="text-[6.5px] text-black">
                              Pass Xingu, Coqueiro| Belém-PA |CEP 66823-335
                            </div>
                          </div>
          ...
          {['colaboradores', 'cadastro', 'qrcode', 'configuracoes'].includes(abaAtiva) && (
            <div className="bg-white p-10 rounded-xl border border-[#cfd8dc] shadow-sm text-center animation-fade-in flex flex-col items-center justify-center min-h-[400px]">
              ...
              <p className="text-slate-500 max-w-md">
                O ecrã de <strong>{menuItens.find((m) => m.id === abaAtiva)?.nome}</strong> será integrado em breve.
              </p>
            </div>
          )}
        </div>
