...
export default function PortalRH() {
  ...
  const getMenuItens = () => {
    ...
    if (usuarioAutenticado.perfil === 'SUPERVISOR') {
      return [ { id: 'emissao', nome: 'Captura & Solicitação', icone: 'fa-camera' } ]; // Supervisor só vê isto
    }
  ...
  const processarSolicitacao = async (solicitacao: any) => {
    // Ao processar, o RH muda de aba, carrega a ficha e a solicitação é marcada como CONCLUÍDO na hora de imprimir
    setBuscaEmissao(solicitacao.matricula_colaborador);
    await buscarColaboradorParaEmissao(solicitacao.matricula_colaborador);
    setAbaAtiva('emissao');
    
    // Atualizamos o status da solicitação para não ficar pendente
  ...
  // ========================== ABA: HUB CENTRAL (COLABORADORES & HISTÓRICO) ==========================
  ...
  const registrarImpressoesEmLote = async () => {
    ...
    try {
      const registros = listaLote.map(c => ({
        matricula_colaborador: c.matricula, nome_colaborador: c.nome_completo, emitido_por: usuarioAutenticado.nome, motivo: 'Emissão em Lote'
      }));
  ...
  const [motivoAcao, setMotivoAcao] = useState('1ª Via'); // Serve para Impressão e para Solicitação
  ...
  // Ação de Impressão com Auditoria (Para RH/ADM)
  ...
  // Ação de Solicitação (Para Supervisor)
  const enviarSolicitacaoAoRH = async () => {
    ...
    try {
      ...
      setColaborador(null); setBuscaEmissao(''); // Limpa a tela após solicitar
  ...
  const carregarUsuarios = async () => {
    try { const response = await fetch(`${URL}/rest/v1/usuarios_sistema?select=*&order=created_at.desc`, { headers: { 'apikey': KEY, 'Authorization': `Bearer ${KEY}` } }); const data = await response.json(); setListaUsuarios(data || []); } catch (e) {}
  };
        ...
        <div className="z-10 w-full max-w-md px-4 sm:px-6">
          <div className="bg-white rounded-2xl overflow-hidden border border-slate-200">
            ...
            <form onSubmit={handleLogin} className="p-6 md:p-8 flex flex-col gap-5 bg-white">
              ...
              <div className="flex items-center justify-between mt-[-5px] mb-2 px-1"><label className="flex items-center gap-2 text-xs font-semibold text-slate-500 cursor-pointer"><input type="checkbox" checked={lembrarLogin} onChange={(e) => setLembrarLogin(e.target.checked)} className="w-3.5 h-3.5 accent-[#023A58]" />Lembrar</label><button type="button" onClick={handleEsqueceuSenha} className="text-xs font-bold text-[#023A58] hover:underline">Esqueceu a senha?</button></div>
              <button type="submit" disabled={carregandoLogin} className="w-full bg-[#023A58] text-white font-semibold py-3.5 rounded-xl hover:bg-[#035B8B] transition-all flex justify-center items-center gap-2">{carregandoLogin ? <><i className="fas fa-spinner fa-spin"></i> Acessando...</> : 'Acessar Sistema'}</button>
            </form>
          </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden print-main-adjust bg-slate-50">
        ...
        <header className="h-20 bg-white shadow-sm border-b border-slate-200 flex items-center justify-between px-6 lg:px-10 z-10 hide-on-print relative">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setMostrarModalSenha(true)} className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-600 transition-colors flex items-center justify-center" title="Alterar Senha"><i className="fas fa-key"></i></button>
          </div>
        </header>
                        ...
                        resultadosBusca.map((colab) => (
                          <tr key={colab.matricula} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                            ...
                            <td className="p-4 text-right pr-6 flex justify-end gap-2">
                              ...
                              <button onClick={() => { setBuscaEmissao(colab.matricula); buscarColaboradorParaEmissao(colab.matricula); }} className="bg-white border border-slate-200 text-[#023A58] hover:bg-slate-100 px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors">Emitir</button>
