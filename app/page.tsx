...
export default function PortalRH() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const user = data[0]; setUsuarioAutenticado(user);
  ...
  // ========================== ABA: HUB CENTRAL & HISTÓRICO DE AUDITORIA ==========================
  ...
  const handleCadastroManual = async (e: React.FormEvent) => {
    ...
    } catch (err: any) { mostrarToast('Erro na inserção.', 'erro'); } finally { setSalvandoCadastro(false); }
  };
  ...
  const handleCriarUsuario = async (e: React.FormEvent) => {
    e.preventDefault(); if (!formUsuario.nome || !formUsuario.login || !formUsuario.senha) { mostrarToast('Preencha tudo.', 'erro'); return; }
    setSalvandoUsuario(true);
    try {
      await fetch(`${URL}/rest/v1/usuarios_sistema`, { method: 'POST', headers: { 'apikey': KEY, 'Authorization': `Bearer ${KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify(formUsuario) });
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden screen-only relative">
      ...
      <aside className="w-72 h-full flex flex-col hide-on-print border-r border-slate-200" style={{ background: '#023A58' }}>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-slate-50">
        <header className="h-20 bg-white shadow-sm border-b border-slate-200 flex items-center justify-between px-6 lg:px-10 z-10 hide-on-print relative">
          <h2 className="text-lg font-bold text-[#023A58]">{menuItens.find(m => m.id === abaAtiva)?.nome || 'Módulo Restrito'}</h2>
