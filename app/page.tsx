...
export default function PortalRH() {
  ...
  // 🔑 CHAVES DIRETO NO CÓDIGO PARA TESTE!
  ...
  const handleBusca = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // A foto só vai aparecer na visualização de impressão.
  ...
  // ==================== FUNÇÃO EXCEL (NOVA E COMPLETA) ====================
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files || !e.target.files[0]) return;
    const file = e.target.files[0];
    setExcelFile(file);
    setMsgFoto({ texto: 'A ler a planilha...', tipo: 'loading' });
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (jsonData.length === 0) {
          setMsgFoto({ texto: 'A planilha está vazia.', tipo: 'erro' });
          return;
        }

        setMsgFoto({ texto: `Importando ${jsonData.length} registros...`, tipo: 'loading' });
        
        // Envia TODOS os dados de uma só vez para o Supabase
        ...
        setMsgFoto({ texto: `${jsonData.length} colaboradores importados com sucesso!`, tipo: 'sucesso' });
        setExcelFile(null);
        carregarTodosColaboradores(); // Atualiza a lista automaticamente
      } catch (error) {
        console.error(error);
        setMsgFoto({ texto: 'Erro ao importar. Verifique as colunas do Excel.', tipo: 'erro' });
      }
    };
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar print-padding-remove">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                  ...
                  <div className="xl:col-span-2 bg-white p-6 rounded-xl border border-[#cfd8dc] shadow-sm">
                    ...
                    <div className="flex flex-col md:flex-row gap-8 justify-center items-center bg-[#f8f9fa] p-8 rounded-lg overflow-x-auto print-container border border-[#eceff1]">
                      ...
                      {/* VERSO COM TIPO SANGUÍNEO (AGORA EM BRANCO SE NÃO TIVER) */}
          ...
          {abaAtiva === 'cadastro' && (
            <div className="bg-white p-10 rounded-xl border border-[#cfd8dc] shadow-sm text-center animation-fade-in flex flex-col items-center justify-center min-h-[400px]">
              ...
              {/* Mensagem de status do Excel */}
              {msgFoto.texto && abaAtiva === 'cadastro' && (
                <p className={`text-xs text-center mt-4 font-bold ${msgFoto.tipo === 'sucesso' ? 'text-[#27ae60]' : msgFoto.tipo === 'erro' ? 'text-[#e74c3c]' : 'text-[#035B8B]'}`}>{msgFoto.texto}</p>
              )}
            </div>
          )}
