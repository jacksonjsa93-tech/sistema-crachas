...
export default function PortalRH() {
  // ========================== ESTADOS GLOBAIS DE UI (NOTIFICAÇÕES) ==========================
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        else { localStorage.removeItem('dinamo_lembrar_login'); setLoginInput(''); }
      } else {
  ...
  const handleEsqueceuSenha = () => {
    mostrarToast("Solicite a redefinição de senha ao Administrador do Sistema.", "aviso");
  };
  ...
  const buscarColaboradorParaEmissao = async (matriculaAlvo: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data && data.length > 0) { 
        setColaborador(data[0]); setFotoCapturada(data[0].foto_url || null); setAbaAtiva('emissao'); setBuscaEmissao('');
  ...
  const handleUploadFoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) { const reader = new FileReader(); reader.onload = (event) => { if (event.target?.result) setRawFoto(event.target.result as string); }; reader.readAsDataURL(e.target.files[0]); }
  };
  ...
  const tentarExcluirUsuario = (id: string, login: string, perfilUsuarioAlvo: string) => {
    ...
    setConfirmDialog({
      ativo: true,
      mensagem: `Deseja realmente excluir o acesso de ${login}?`,
      acao: async () => {
        try { 
        ...
        <div className="z-10 w-full max-w-md px-4 sm:px-6">
          <div className="bg-white rounded-2xl overflow-hidden border border-slate-200">
            <div className="p-8 flex flex-col items-center bg-white border-b border-slate-100">
              ...
              <h2 className="text-xl md:text-2xl font-bold tracking-tight text-[#023A58]">Portal Operacional</h2>
              <p className="text-xs md:text-sm text-slate-500 mt-1 font-medium">Acesso restrito à gestão</p>
            </div>
            ...
            <form onSubmit={handleLogin} className="p-6 md:p-8 flex flex-col gap-5 bg-white">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={loginInput} onChange={(e) => setLoginInput(e.target.value.toLowerCase().replace(/\s/g, ''))} placeholder="Login de acesso" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-sm focus:outline-none focus:border-[#023A58] font-medium text-slate-800 transition-all lowercase" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="password" value={senhaInput} onChange={(e) => setSenhaInput(e.target.value)} placeholder="••••••••" className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-sm focus:outline-none focus:border-[#023A58] font-medium text-slate-800 transition-all" />
                </div>
              </div>

              <div className="flex items-center justify-between mt-[-5px] mb-2 px-1">
                <label className="flex items-center gap-2 text-xs font-semibold text-slate-500 cursor-pointer hover:text-slate-700 transition-colors">
                  <input type="checkbox" checked={lembrarLogin} onChange={(e) => setLembrarLogin(e.target.checked)} className="w-3.5 h-3.5 accent-[#023A58]" />
                  Lembrar login
                </label>
                <button type="button" onClick={handleEsqueceuSenha} className="text-xs font-bold text-[#023A58] hover:underline transition-colors">
                  Esqueceu a senha?
                </button>
              </div>

              <button type="submit" disabled={carregandoLogin} className="w-full bg-[#023A58] text-white font-semibold py-3.5 rounded-xl hover:bg-[#035B8B] transition-all flex justify-center items-center gap-2">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden print-main-adjust bg-slate-50">
        ...
        <header className="h-20 bg-white shadow-sm border-b border-slate-200 flex items-center justify-between px-6 lg:px-10 z-10 hide-on-print relative">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg md:text-xl font-bold text-[#023A58] hidden sm:flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3 bg-white px-2 pr-4 py-1.5 rounded-full border border-slate-200 shadow-sm hidden sm:flex">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[9px] font-bold uppercase tracking-widest text-[#0a84ff]">Perfil: {usuarioAutenticado.perfil}</span>
              </div>
            </div>
                ...
                <form onSubmit={handlePesquisaTabela} className="flex flex-col md:flex-row gap-4 items-end">
                  <div className="flex-1 w-full">
                    ...
                    <div className="relative">
                      ...
                      <input type="text" placeholder="Digite a Matrícula ou Nome..." value={buscaTabela} onChange={(e) => setBuscaTabela(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-sm focus:outline-none focus:border-[#023A58] transition-all" />
                    </div>
                  </div>
              ...
              <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200">
                <form onSubmit={adicionarAoLote} className="flex flex-col md:flex-row gap-4 items-end">
                  <div className="flex-1 w-full">
                    ...
                    <input type="text" placeholder="Matrícula..." value={matriculaLote} onChange={(e) => setMatriculaLote(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#023A58] transition-all" />
                  </div>
            ...
            <div className="max-w-full lg:max-w-6xl mx-auto hide-on-print animation-fade-in">
              <form onSubmit={(e) => { e.preventDefault(); buscarColaboradorParaEmissao(buscaEmissao); }} className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200 mb-6 flex flex-col md:flex-row gap-4 items-end">
                <div className="flex-1 w-full">
                  ...
                  <input type="text" placeholder="Ex: 6294" value={buscaEmissao} onChange={(e) => setBuscaEmissao(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-[#023A58] transition-all" />
                </div>
                <button type="submit" disabled={carregandoEmissao} className="w-full md:w-auto bg-[#023A58] text-white font-semibold px-8 py-3 rounded-xl hover:bg-[#035B8B] shadow-sm transition-all">Buscar Ficha</button>
              </form>
              ...
              {colaborador && (
                <div className="flex flex-col lg:flex-row gap-6">
                  ...
                  <div className="w-full lg:flex-1 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row justify-center bg-slate-50 p-6 rounded-xl border border-slate-100 overflow-x-auto gap-6">
                      {/* CARTÃO FRENTE */}
